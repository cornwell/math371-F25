...
Say have data $S=\{(x_i, y_i)\}_{i=1}^P$ and want to fit a polynomial to this in the plane. That is, we pick a degree $d$ and will use a polynomial function 
        \[w^\star_dx^d + w^\star_{d-1}x^{d-1} + \ldots + w^\star_1x + b^\star\]
(so, $\tilde{\bf w}^\star = (b^\star, w^\star_1, \ldots, w^\star_d)$ is our solved stationary point from the normal equation). 

What happens when we choose $d$ to be rather large? \onslide<3-> $d = 12$

\begin{figure}[h]
    \includegraphics[width=0.45\textwidth]{../../Images/data_polyfit_deg12.png}
    \caption{Have blue points as data; will train on them.}
\end{figure}
...
\begin{figure}[h]
    \includegraphics[width=0.45\textwidth]{../../Images/unseendata_polyfit_deg12.png}
    \caption{Orange points are from same population but weren't in sample.}
\end{figure}
...
The cost function you compute is \emph{empirical} (based on observed data). We discuss a (somewhat absurd) example to demonstrate the point that 
\begin{center} small counting cost $\centernot\implies$ high accuracy on ``yet unseen'' data\end{center}
...
\begin{frame}
    ...
    You \textbf{are not supposed to touch the test data} until your model is finished and you are evaluating it.
